...
readonly RUN_ID="019_perf_hcs_v0_budget20_no_activity_fr1desk_full_r1"
...
readonly HISTORY_BUDGET="20"
readonly RUN_ORDER_POSITION="SECOND_AFTER_018_REVIEW"
readonly REQUIRES_018_REVIEW_PASS="true"
readonly REVIEW_018_RECORD="$ROOT/records/018_perf_hcs_v0_budget10_no_activity_fr1desk_full_r1"
readonly REVIEW_018_GATE="$REVIEW_018_RECORD/018_REVIEW_PASS.txt"
...
test "$REQUIRES_018_REVIEW_PASS" = "true" \
    || die "019_REQUIRES_018_REVIEW_PASS is not true"
test -f "$REVIEW_018_GATE" \
    || die "018 review gate is missing: $REVIEW_018_GATE"
test "$(grep -Ec '^018_REVIEW_PASS=true$' "$REVIEW_018_GATE")" -eq 1 \
    || die "018 review gate does not contain the required line exactly once"
test "$(grep -Ecve '^[[:space:]]*$|^018_REVIEW_PASS=true$' "$REVIEW_018_GATE")" -eq 0 \
    || die "018 review gate contains unexpected content"
...
sha256sum "$REVIEW_018_GATE" > "$REC/018_REVIEW_PASS.sha256"
...
if errors:
    raise SystemExit(1)
print("019_HCS_V0_BUDGET20_CLEAN_PERFORMANCE_VALIDATION_PASS")
PY
...
echo "019_HCS_V0_BUDGET20_CLEAN_PERFORMANCE_RUN_COMPLETE"
